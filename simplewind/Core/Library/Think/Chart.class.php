...
class Chart {
  ...
  */
  ...
  static function create3dpie($title,$data=array(),$size=40,$height=100,$width=80,$legend=array(),$slice=0,$img_name){
      //包含相关的文件
	  ...
	  //$graph->Stroke();
	  //保存整福统计图
	  $path = './data/runtime/Cache/'.$img_name;
	  $graph->Stroke($path);
  }	
   ...
   * 设置柱状图
   */
  static function createcolumnar($title,$data=array(),$size=40,$height=100,$width=80,$legend=array(),$img_name){
  	    //载入文件
		vendor("Jpgraph.jpgraph");
		...
		//$graph->yaxis->title->Set("Number of People");
